...
(function(){
	...
	.factory("opUtil",function($mdDialog){
           
		var _openItem = function(item, callback){
			
			$mdDialog.show({
				animation: true,
				templateUrl:"app/op/html/detalhe.html",
				size:'lg',
				controllerAs:"vm",
				controller: function(stService, stUtil, $modalInstance, $route){
					...
					vm.salvarItem = function(){
						...
						stService.executePost("op/add/", _item).then(function(data){
					...
					vm.deletarItem = function(){
						...
						stService.executePost("op/delete/", [vm.item.id]).then(function(){
