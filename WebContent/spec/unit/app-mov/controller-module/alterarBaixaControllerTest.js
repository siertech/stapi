...
describe('Testes alterarBaixaController', function () {
	...
	it("Baixar movimentação parcelada com pedidos", inject(function(){
	   ...
	   var mov = {
		...
		var vm = $controller("alterarBaixaController",{modal:$mdDialogInstance,mov:mov,callback:null});
		$httpBackend.whenPOST("http://server/movimentacao/add/",mov).respond("");
        vm.confirmar($mdDialogInstance,vm.dataPagamento,vm.valorMov);
        $httpBackend.flush();
        expect($mdDialogInstance.$dismiss).toHaveBeenCalled();
